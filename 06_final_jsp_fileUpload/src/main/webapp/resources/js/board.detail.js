...
async function removeTotalFileFromServer(bnoVal) {
  try {
    const url = "/file/removeAll";
    const config = {
      method : 'post',
      headers : {
        'Content-Type' : 'application/json; charset=utf-8'
      },
      body : JSON.stringify(bnoVal)
    };
    const resp = await fetch(url, config);
    const result  = await resp.text();
    return result;
...
document.getElementById('fileTotalDelBtn').addEventListener('click', ()=>{
  removeTotalFileFromServer(bnoVal).then(result =>{
    if(result > 0 ){
      alert('전체 삭제 성공!');
    }
  })
